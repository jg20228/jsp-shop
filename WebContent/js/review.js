    ...
    window.power_review_write = {
                            ...
                            if (page_type == 'product' || page_type == 'detail' || page_type == 'write' || page_type == 'update') {
                                if (action == 'update') {
                                    ...
                                    if (config.mobile_use && config.mobile_use == 1) {
                                        ...
                                        if (page_type == 'write') {
                                            location.href = "/board/power_review_product.html?product_uid=" + config.product_uid;
                                        }
                                    }
                                }
                            }
        ...
        fileUpDone: function(file_name) {
            ...
            if (file_name && file_name.length > 0) {
                ...
                var _img_path = config.board_host + '/board/tmp/'+ config.file_board_code + ':mini::' + file_name; 
    ...
    window.power_review_main = {
        ...
        change_product: function (ordernum, basketnum) {
            ...
            $.ajax({
                ...
                success : function(response) {
                    if (response.success == true) {
                        if (config.main_template && config.main_template == '2' && response.sdata && response.sdata.length > 0) {
    ...
    window.power_review_list = {
        ...
        good_review: function (num1, old, page_type) {
            ...
            if (power_review_pack.info.config.mobile_use == 1 && page_type == 'view') {
                ...
                _m_tar = $('#PR_GOOD strong');
            }
            var param = 'action_type=good_review&num1=' + num1 + '&old=' + old + '&page_type=' + page_type + '&good_chk=' + good_chk + '&bad_chk=' + bad_chk;
        ...
        bad_review: function (num1, old, page_type) {
            ...
            var param = 'action_type=bad_review&num1=' + num1 + '&old=' + old + '&page_type=' + page_type + '&good_chk=' + good_chk + '&bad_chk=' + bad_chk;
            $.ajax({
                type : 'post',
                url  : power_review_pack.info.location_dir + 'power_review.action.html',
                data : param,
        ...
        view_comment: function (num1, action) {
            ...
            if (power_review_pack.info.config.mobile_use != 1) {
                if (config.info_comment_view == 'N' || (action != '' && config.info_comment_view == 'Y' && page_type == 'detail')) {
